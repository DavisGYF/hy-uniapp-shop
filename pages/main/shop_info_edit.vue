        ...
        <form @submit="formSubmit" @reset="formReset">
          <view class="uni-list">
            <view class="uni-list-cell">
              <view class="uni-title" style="color: #777;">基本信息</view>
            </view>
            <view class="uni-list-cell">
              店铺名称
            ...
            <!-- <view class="uni-list-cell">
            	营业范围
            	<view class="uni-list-cell-right">
            		<input class="uni-input"  name="label" placeholder="请输入" />
            	</view>
            </view> -->
            <!-- <view class="uni-list-cell">
              营业状态
                  ...
                  <view class="uni-uploader__files">
                    <block>
                      <view class="uni-uploader__file">
                        <image class="uni-uploader__img" @tap="previewImagFn(text_info.avatar)" mode="aspectFill" :src="text_info.avatar"
                          :data-src="text_info.avatar"></image>
                      </view>
                    </block>
        ...
        <form @submit="formSubmit" @reset="formReset">
          <view class="uni-list">
            ...
            <view class="mapWrap">
              ...
              <view class="infoWarp">
                <view class="uni-title">店铺地址</view>
                <view class="">
									...
									<view class="uni-uploader__files">
										<block>
											<view class="uni-uploader__file" style="width: 60%;">
												<image class="uni-uploader__img" @tap="previewImagFn(text_info.store_img)" mode="aspectFill" :src="text_info.store_img" :data-src="text_info.store_img" ></image>
											</view>
										</block>
                  ...
                  <view class="uni-uploader__files">
                    <!-- <block v-for="(image,index) in imageList" :key="index"> -->
                      <view class="uni-uploader__file" v-if="text_info.store_img1!='' ">
                        <image class="uni-uploader__img" mode="aspectFill" :src="text_info.store_img1" :data-src="text_info.store_img1" @tap="previewImagFn(text_info.store_img1)"></image>
                      </view>
                    <!-- </block> -->
                    <view class="uni-uploader__input-box" v-if="text_info.status!=1 " ><!-- v-if="text_info.store_img1=='' " -->
                      <!-- /上传照片框框 -->
								...
								<view class="uni-uploader-body">
									<view class="uni-uploader__files">
										<block v-for="(image,index) in id_image_ist" :key="index">
											<view class="uni-uploader__file">
												<image  v-if="text_info.status==1" class="uni-uploader__img" @tap="previewImagFns(image,index,1)" mode="aspectFill" :src="image" :data-src="image" ></image>
												<image v-if="text_info.status!=1"  class="uni-uploader__img" @tap="previewImagFns(image,index)" mode="aspectFill" :src="image" :data-src="image" ></image>
											</view>
										</block>
										<view class="uni-uploader__input-box"  v-if="text_info.status!=1"><!-- /上传照片框框 -->
            ...
            <view class="uni-list-cell">
              身份证号
              <view class="uni-list-cell-right">
                <input class="uni-input" :disabled="text_info.status==1" v-model="text_info.id_card" type="idcard" name="id_card" placeholder="请输入" />
              </view>
            </view>
...
//         ],
		...
		onShow() {
			var _this=this
        console.log('---------store show===text_info',this.$store.state.shop_edit_info.length,JSON.stringify(this.$store.state.shop_edit_info))
        if(_this.text_info){
          ...
          }else if(_this.text_info.profile_picture1==undefined&&_this.text_info.profile_picture2==undefined){       console.log('stroe--->','00')
           _this.id_image_ist = []
          }else{
            console.log('stroe--->',2)
                ...
                console.log("res?-=======", JSON.stringify(res));
                  ...
                  uni.uploadFile({
                    ...
                    success: (res) => {
                      ...
                      _this.text_info.avatar='https://ffomall.oss-cn-hangzhou.aliyuncs.com/'+JSON.parse(res.data).filename
                    },
                    fail: (err) => {
             ...
             success: (res) => {
               ...
               console.log("res?-=======", JSON.stringify(res));
                 ...
                 uni.uploadFile({
                   ...
                   success: (res) => {
                     console.log('uploadImage success, res is:', res.data)
                     _this.text_info.store_img='https://ffomall.oss-cn-hangzhou.aliyuncs.com/'+JSON.parse(res.data).filename
                   },
                   fail: (err) => {
                     console.log('uploadImage fail', JSON.stringify(err));
                     uni.showModal({
              ...
              success: (res) => {
                ...
                console.log("res?-=======", JSON.stringify(res));
                  ...
                  uni.uploadFile({
                    ...
                    success: (res) => {
                      console.log('uploadImage success------, res is:', res.data)
                      _this.text_info.store_img1='https://ffomall.oss-cn-hangzhou.aliyuncs.com/'+JSON.parse(res.data).filename
      ...
      chooseImageFns(param){
        ...
        uni.chooseImage({
          ...
          success: (res) => {
            ...
            for (let i in files) {
              let fileName = _this.uploaddata_id.dir + Date.parse(new Date()) + '_' + _this.randomString(6) + '.' + files[i].split(".").pop();
              uni.uploadFile({
                ...
                success: (res) => {
                  ...
                  _this.id_image_ist.push('https://ffomallcardid.oss-cn-hangzhou.aliyuncs.com/'+JSON.parse(res.data).filename)
      ...
      getLocation() {
        ...
        this.$util.ajax_uni({
          ...
          success: function(res) {
            ...
            if (res.data.status == 1) {
              uni.navigateBack({url:'./shop_info'})
